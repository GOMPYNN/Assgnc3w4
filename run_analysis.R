# Use package downloader for downloading the zipped file
library(downloader)
...
# UCI HAR Dataset directory created containing the unzipped files
...
# Select the mean and standard deviation features
...
# Remove "-" and "()" from the names
featuresNames <- gsub("-","", featuresNames)
featuresNames <- gsub("\\(+\\)","", featuresNames)

# Load the train datasets
trainSet <- read.table("./data/UCI HAR Dataset/train/X_train.txt")[featuresSelected]
...
testSet <- read.table("./data/UCI HAR Dataset/test/X_test.txt")[featuresSelected]
